// ANCHOR inpost gratis badge

jQuery('document').ready(function() {

    if (document.body.classList.contains('woocommerce-checkout')) {

        var shippings = document.querySelectorAll('.woocommerce-shipping-methods li label');

        shippings.forEach(element => {
            if (element.getAttribute('for').includes('free')) {
                element.classList.add('free-shipping');
            };
        });
...
jQuery('document').ready(function() {
    ...
    const imgs = document.querySelectorAll('.custom-gallery img');
    ...
    if ((imgs.length % 2) !== 0) {
        document.querySelector('.custom-gallery div:last-of-type').classList.add('wide-img');
    }
...
jQuery('document').ready(function() {
    ...
    if (document.body.classList.contains('single-product')) {
        ...
        var lightbox = GLightbox({
            selector: '.woocommerce-product-gallery__wrapper > .woocommerce-product-gallery__image > a, .custom-gallery a',
            loop: true,
            preload: false,
            openEffect: 'fade',
            closeEffect: 'fade'
