    ...
    if($products) {  
        ...
        foreach($products->products as $product) {
            ...
            if ($current_tax) { // dla widoku katalogu
                ...
                } else { // dla widoku wynikow wyszukiwania i sklepu
            ...
            <?php if($filters['first_term']) : ?>
                    ...
                    <div class="first-term">
                        ...
                        <?php 
                            echo wp_sprintf(
                                '<div><span>%s</span>',
                                $filters['first_term']['tax'] == 'kolekcje' ? __('Kolekcja', 'decobelo') : __('Produkt', 'decobelo')
                            );

                                echo sprintf(
                                    '<ul data-filter="%s" data-title="%s">',
                                    $filters['first_term']['tax'],
                                    $filters['first_term']['tax'] == 'kolekcje' ? __('Kolekcja', 'decobelo') : __('Produkt', 'decobelo')
                                );

                                foreach ($filters['first_term']['terms'] as $term) {
                                    echo wp_sprintf(
                                        '<li>
                                            <a href="%s" data-value="%s" data-type="%s" class="filter">%s</a>',
                                        $filters['first_term']['tax'] == 'kolekcje' ? add_query_arg('kolekcja', $term['slug']) : add_query_arg('kategoria', $term['slug']),
                                        $term['slug'],
                                        $filters['first_term']['tax'],
                                        $term['name']
                                    );
